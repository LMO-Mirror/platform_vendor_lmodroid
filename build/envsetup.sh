...
function lmoremote()
{
    ...
    local LMO_USER=$(git config --get review.gerrit.libremobileos.io.username)
    if [ -z "$LMO_USER" ]
    then
        git remote add lmogerrit ssh://gerrit.libremobileos.io:29418/$PFX$PROJECT
    else
        git remote add lmogerrit ssh://$LMO_USER@gerrit.libremobileos.io:29418/$PFX$PROJECT
    fi
...
function lmodroidremote()
{
    ...
    git remote add lmodroid https://git.libremobileos.io/LMODroid/$PROJECT
...
function lmogerrit() {
    ...
    local user=`git config --get review.gerrit.libremobileos.io.username`
...
function lmorebase() {
    ...
    cd $dir
    repo=$(cat .git/config  | grep git://git.libremobileos.io | awk '{ print $NF }' | sed s#git://git.libremobileos.io/##g)
    echo "Starting branch..."
    repo start tmprebase .
    echo "Bringing it up to date..."
    repo sync .
    echo "Fetching change..."
    git fetch "http://gerrit.libremobileos.io/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
