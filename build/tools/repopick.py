...
if __name__ == '__main__':
    # Default to LibreMobileOS Gerrit
    default_gerrit = 'https://gerrit.libremobileos.io'

    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
        repopick.py is a utility to simplify the process of cherry picking
        patches from LibreMobileOS's Gerrit instance (or any gerrit instance of your choosing)
    ...
    for item in mergables:
        ...
        # Try fetching from LMO git first if using default gerrit
        if args.gerrit == default_gerrit:
            if args.verbose:
                print('Trying to fetch the change from LMO git')

            if args.pull:
                cmd = ['git pull --no-edit lmodroid', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch lmodroid', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
                print(cmd)
        ...
        if args.gerrit != default_gerrit or os.stat(FETCH_HEAD).st_size == 0:
            # If not using the default gerrit or LMO git failed, fetch from gerrit.
            if args.verbose:
                if args.gerrit == default_gerrit:
                    print('Fetching from LMO git didn\'t work, trying to fetch the change from Gerrit')
                else:
