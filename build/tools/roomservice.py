...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from LMODroid-Devices of LMO Git (https://git.libremobileos.io/LMODroid-Devices)." % device)

repositories = []

try:
    authtuple = netrc.netrc().authenticators("git.libremobileos.io")
...
if not depsonly:
    gitlmoreq = urllib.request.Request("https://git.libremobileos.io/api/v4/groups/LMODroid-Devices")
    add_auth(gitlmoreq)
    try:
